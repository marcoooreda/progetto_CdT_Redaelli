...
<xsl:stylesheet 
	...
	<xsl:template match="tei:sourceDesc">
		...
		<xsl:for-each select="tei:biblStruct/tei:analytic">
			<div class="articolo">
				...
				<xsl:value-of select="../tei:monogr/tei:biblScope[@corresp = concat('#', $id)]"/>
                lingua:
	...
	<xsl:template match="tei:div[@type='article']">
		...
		<div class="box-container" id="box-{$articleId}">
            ...
            <!--APPLY TEMPLATES DEL DEI PB DENTRO GLI ARTICOLI-->
	...
	<xsl:template match="tei:div[@type='bibliography']">
		...
		<div class="box-container" id="box-{$bibliographyId}">
            ...
            <!--APPLY TEMPLATES DEI PB DENTRO BIBLIOGRAFIA-->
			<xsl:apply-templates select="tei:p/tei:pb"/>
		</div>
	</xsl:template>
    <!--TEI NEWS-->
	<xsl:template match="tei:div[@type='news']">
		...
		<div class="box-container" id="box-{$newsId}">
            ...
            <!--APPLY TEMPLATES DEI PB DENTRO NEWS-->
			<xsl:apply-templates select="tei:p/tei:pb"/>
		</div>
	</xsl:template>
    <!--TEI PAGE-->
	<xsl:template match="tei:pb[@type='page']">
        <!--DICHIARO LA VARIABILE CON L'ID DEL PB PAGE-->
		<xsl:variable name="pageId" select="@xml:id"/>
		<xsl:variable name="np" select="@n"/>
        <!--CREO BOX CONTAINER CON ID: BOX- + VARIABILE-->
		<div class="box-container-articoli" id="page-{$pageId}">
			<div class="columns-wrapper">
                <!--APPLY TEMPLATES DELLA COLONNA SINISTRA-->
				<xsl:apply-templates select="following-sibling::tei:cb[@type='sectionSX']"/>
                <!--APPLY TEMPLATES DELLA COLONNA DESTRA-->
				<xsl:variable name="next_pb" select="(parent::tei:p/following-sibling::tei:p/tei:pb)[1]"/> <!-- salvo il primo pb prossimo nella gerarchia -->
				<!-- ora cerco tutte le prossime cb_dx prima della prossima pagina -->
				<!-- in concreto: salgo al paragrafo che mi contiene. guardo i successivi finchè non trovo nuovi cb_dx: checko che o non c'è un pb successivo (è finita la pagina) oppure filtro solo quelli che vengono prima di quel pb (che non siano della prossima pagina) -->
				<!-- riesco a fare ciò perchè ho letto che xsl restituisce un id coerente con l’ordine di apparizione nel documento, quindi faccio un confronto se il mio id del cb è minore dell'id della prossima pagina -->
				<xsl:apply-templates select="parent::tei:p/following-sibling::tei:p/tei:cb[@type='sectionDX'][not($next_pb) or generate-id() &lt; generate-id($next_pb)]"/> <!--   &lt; sta per <   -->
			</div>
		</div>
	</xsl:template>
    <!--TEI COLONNA SINISTRA-->
	<xsl:template match="tei:cb[@type='sectionSX']">
		<xsl:variable name="n" select="@n"/>
        <!--DICHIARO LA VARIABILE CON L'ID DEL CB COLONNA SINISTRA-->
		<xsl:variable name="sectionId" select="@xml:id"/>
        <!--CREO UN DIV CON ID DELLA VARIABILE-->
		<div class="section-left" id="{$sectionId}">
			<!-- in caso che è la prima colonna negli articoli c'è l'head -->
			<xsl:if test="$n=1">
				<h1>
                    <!--ESTRAGGO L'HEAD
			...
			<xsl:for-each select="ancestor::tei:div/tei:p"> <!-- seleziono tutti i p e uno alla volta: -->
				<xsl:choose> <!-- abbiamo 2 casi -->
				<xsl:when test="substring-before(@rend, '_')='frammentato'"> <!-- è un p frammentato? ulteriori 2 casi: -->
					<xsl:if test="$n=substring-after(@rend, '_')"> <!-- 1) se il numero dopo frammentato è la colonna in cui siamo devo stampare la prima parte del paragrafo -->
						<!-- voglio la prima parte stampata-->
						<xsl:apply-templates select=".">
							<xsl:with-param name="parte" select="'prima_parte'"/> <!-- passo come parametro alla funzione apply-templatesche voglio la prima parte -->
						</xsl:apply-templates>
					</xsl:if>
					<xsl:if test="number($n - 1) = number(substring-after(@rend, '_'))"> <!-- 2) altrimenti se il numero dopo frammentato è la colonna precedente devo comunque prenderlo ma solo la seconda parte (perchè se è iniziato nella colonna precedente ed è frammentato termina all'inizio della colonna attuale). -->
					<!-- voglio la seconda parte stampata-->
						<xsl:apply-templates select=".">
							<xsl:with-param name="parte" select="'seconda_parte'"/> <!-- passo come parametro che voglio la seconda parte -->
						</xsl:apply-templates>
					</xsl:if>
				</xsl:when>
				<xsl:otherwise> <!-- se non è frammentato -->
					<xsl:if test="substring-after(@rend, '_') = $n"> <!-- applico tutti quelli della mia colonna -->
    					<xsl:apply-templates select="."/>
  					</xsl:if>
				</xsl:otherwise>
				</xsl:choose>
			...
			<!-- aggiungo le note -->
            <xsl:if test="ancestor::tei:div/tei:p/tei:note[substring-after(@rend, '_')=$n]"> <!-- checko che siano della mia colonna -->
				<div class="box-note">
					<xsl:for-each select="ancestor::tei:div/tei:p/tei:note[substring-after(@rend, '_')=$n]">
						<div class="nota">
							<xsl:apply-templates/>
						</div>
					</xsl:for-each>
	...
	<xsl:template match="tei:cb[@type='sectionDX']">
		<xsl:variable name="n" select="@n"/>
		<xsl:variable name="sectionId" select="@xml:id"/>
		<div class="section-right" id="{$sectionId}">
			<xsl:if test="ancestor::tei:div[@type='bibliography']/tei:p/tei:pb[@rend='prima_pagina_bibliografia'] and $n=2">
				<h1>
					<xsl:apply-templates select="ancestor::tei:div[@type='bibliography']/tei:head"/>
				</h1>
			</xsl:if>
			<xsl:if test="ancestor::tei:div[@type='news']/tei:p/tei:pb[@type='page'][@rend='prima_pagina_notizie'] and $n=2">
				<h1>
					<xsl:apply-templates select="ancestor::tei:div[@type='news']/tei:head"/>
				</h1>
			</xsl:if>

			<xsl:for-each select="ancestor::tei:div/tei:p">
				<xsl:choose>
				<xsl:when test="substring-before(@rend, '_')='frammentato'">
					<xsl:if test="$n=substring-after(@rend, '_')">
						<xsl:apply-templates select=".">
							<xsl:with-param name="parte" select="'prima_parte'"/>
						</xsl:apply-templates>
					</xsl:if>
					<xsl:if test="number($n - 1) = number(substring-after(@rend, '_'))">
						<xsl:apply-templates select=".">
							<xsl:with-param name="parte" select="'seconda_parte'"/>
						</xsl:apply-templates>
					</xsl:if>
				</xsl:when>
				<xsl:otherwise>
					<xsl:if test="substring-after(@rend, '_') = $n">
    					<xsl:apply-templates select="."/>
  					</xsl:if>
				</xsl:otherwise>
				</xsl:choose>
			...
			<xsl:if test="ancestor::tei:div/tei:p/tei:note[substring-after(@rend, '_')=$n]">
				<div class="box-note">
					<xsl:for-each select="ancestor::tei:div/tei:p/tei:note[substring-after(@rend, '_')=$n]">
						<div class="nota">
							<xsl:apply-templates/>
	...
	<xsl:template match="tei:p">
		<xsl:param name="parte"/> <!-- controllo se ho ricevuto il parametro che ho passato da cb -->
		<xsl:variable name="nc" select="substring-after(@rend, '_')"/> <!-- variabile del numero di colonna di inizio paragrafo -->
		<xsl:variable name="fram" select="substring-before(@rend, '_')"/> <!-- variabile che contiene se è frammentato -->
		<xsl:if test="$fram!='frammentato'"> <!-- se non è frammentato applico tutto ciò che contiene il paragrafo tranne note (che ho già estratto), le pb e la prima cb perchè già lo faccio a monte -->
			<p>
				<xsl:apply-templates select="node()[not(self::tei:note or self::tei:pb or self::tei:cb[1])]"/>
			</p>
		</xsl:if>
		<xsl:if test="$fram='frammentato'"> <!-- se è frammentato -->
		<xsl:if test="$parte='prima_parte'"> <!-- se è la prima parte -->
			<!-- contenuto prima del cb -->
			<p>
				<xsl:apply-templates select="node()[following-sibling::tei:cb][not(self::tei:pb)]"/> <!--applico tutto ciò che dentro il paragrafo dopo ha un cb (a parte eventuale pb) ovvero i nodi che hanno un cb dopo di loro -->
			</p>
		</xsl:if>
		<xsl:if test="$parte='seconda_parte'"> <!-- se è la seconda parte -->
			<!-- contenuto dopo il cb -->
			<p>
				<xsl:apply-templates select="node()[preceding-sibling::tei:cb]"/> <!-- applico tutto ciò che dentro il paragrafo ha precedentemente un cb ovvero i nodi che hanno un cb prima di loro -->
			</p>
		</xsl:if>
		</xsl:if>
	</xsl:template>
			...
			<xsl:if test="$gloss">
				...
				<span>
					...
					<xsl:attribute name="class">
                        <!-- verifico se il primo cb precedente è di tipo dx o sx-->
						<xsl:choose>
							<xsl:when test="preceding::tei:cb[1]/@type='sectionDX'">infospanDX</xsl:when>
	...
	<xsl:template match="tei:rs[@type='law']">
		<xsl:variable name="gloss_corresp" select="substring-after(@ref, '#')" />
		<xsl:variable name="unique_id" select="generate-id()" />
		<span class="law">
			<xsl:attribute name="id">
				<xsl:value-of select="$unique_id"/>
    ...
    <!--TEI tei:placeName | tei:country[@ref='#Italia'] STESSA LOGICA DI SEG TYPE="need_explanation" MA CON @ref-->
	<xsl:template match="tei:placeName[@ref] | tei:country[@ref]">
		<xsl:variable name="place_corresp" select="substring-after(@ref, '#')" />
		<xsl:variable name="unique_id" select="generate-id()" />
		<span class="place">
			<xsl:attribute name="id">
				<xsl:value-of select="$unique_id"/>
	...
	<xsl:template match="tei:persName[@type='real']">
		<xsl:variable name="person_corresp" select="substring-after(@ref, '#')" />
		<xsl:variable name="unique_id" select="generate-id()" />
	...
	<xsl:template match="tei:zone">
        <!--DICHIARO LA VARIABILE IMGWIDTH A SECONDA DELLA PAGINA:
        IN REND LA ZONE DIAMO WIDTH DIVERSE PERCHE' LA WIDTH DELL'IMMAGINE CAMBIA PER OGNI IMMAGINE-->
		<xsl:variable name="imgWidth" select="number(substring(../tei:graphic/@width, 1, string-length(../tei:graphic/@width) - 2))"/>
        <!--DICHIARO LA VARIABILE IMGHEIGHT A SECONDA DELLA PAGINA: 
        IN REND LA ZONE DIAMO HEIGHT DIVERSE PERCHE' LA HEIGHT DELL'IMMAGINE CAMBIA PER OGNI IMMAGINE-->
		<xsl:variable name="imgHeight" select="number(substring(../tei:graphic/@height, 1, string-length(../tei:graphic/@height) - 2))"/>
